...
def main(driver_id=None):
	driver = get_driver(driver_id) if driver_id is not None else None

	if driver_id is not None and driver is None:
	...
	session_manager = SessionManager(driver_id) if driver_id is not None else None
	...
	print(f"Driver Attention Momentum System Running for {driver_label} (ESC to quit)")
...
def parse_args():
	parser = argparse.ArgumentParser(description="Driver attention monitoring runtime")
	parser.add_argument(
		"--driver-id",
		type=int,
		default=None,
		help="Attach monitoring and event logs to a specific driver profile.",
	)
